...
- [X] T064 [US2] FDS에서 중간 위험도 거래 시나리오 검증 (위험 점수 40-70점)
- [X] T065 [US2] 추가 인증 성공 시나리오 검증 (거래 승인)
- [X] T066 [US2] 추가 인증 3회 실패 시나리오 검증 (거래 차단)
...
- [X] T071 [US3] 고위험 거래 자동 차단 로직 구현 (위험 점수 80-100점)
- [X] T072 [US3] ReviewQueue 모델 생성 (services/fds/src/models/review_queue.py)
- [X] T073 [US3] 차단된 거래를 수동 검토 큐에 자동 추가하는 로직 구현
...
- [X] T083 [US3] 악성 IP 접속 시 자동 차단 시나리오 검증
- [X] T084 [US3] 보안팀 대시보드 알림 수신 검증
- [X] T085 [US3] 수동 검토 및 차단 해제 플로우 검증
...
- [X] T096 [US4] 상품 CRUD 작업 검증
- [X] T097 [US4] 주문 상태 변경 및 고객 알림 검증
...
- [X] T126 [US6] 모델 재학습 파이프라인 검증
- [X] T127 [US6] 카나리 배포 및 롤백 시나리오 검증
