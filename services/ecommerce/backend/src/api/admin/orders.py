...
class OrderListResponse(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            "example": {"orders": [], "total_count": 100, "page": 1, "page_size": 20}
        }
...
async def get_orders(
    ...
    **반환**:
    ...
    return OrderListResponse(
        orders=orders, total_count=total_count, page=page, page_size=page_size
    )
    ...
    try:
        if new_status == OrderStatus.PAID:
            ...
            if old_status not in [OrderStatus.PAID]:
                raise ValueError(
                    f"'{old_status.value}' 상태에서 '{new_status.value}'로 변경할 수 없습니다."
                )
            ...
            if old_status != OrderStatus.DELIVERED:
                raise ValueError(
                    f"'{old_status.value}' 상태에서 환불할 수 없습니다. 배송 완료 후에만 환불 가능합니다."
                )
