...
async def get_current_user(
    ...
    # TODO: JWT 인증 미들웨어 통합
    raise HTTPException(
        status_code=status.HTTP_501_NOT_IMPLEMENTED, detail="JWT 인증 통합 예정"
    )
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "purpose": "transaction",
                "metadata": {"order_id": "ORD-20250114-123456", "amount": 150000},
            }
        }
...
async def request_otp(request: OTPRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        result = await otp_service.generate_otp(
            user_id=request.user_id, purpose=request.purpose, metadata=request.metadata
        )
...
async def verify_otp(request: OTPVerifyRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        result = await otp_service.verify_otp(
            user_id=request.user_id, otp_code=request.otp_code, purpose=request.purpose
        )
